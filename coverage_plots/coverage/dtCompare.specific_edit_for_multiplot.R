...
# THIS VERSION OF THE CODE HAS BEEN SPECIFICALLY EDITED TO PLOT MULTIPLE PROMOTER SETS
# TO THE SAME PANEL.
# For the much of the functionality described below please use dtCompare.R
...
for(fileName in files){
	...
	categories <- read.delim(pipe(paste("head -1", fileName, "| sed 's/#//' | perl -pe 's/([^\t]+):([0-9]+)\t*/\\1\t\\2\n/g'")), header=F, col.names=c("Name", "number"))
	dimensions <- read.delim(pipe(paste("head -2", fileName, "| tail -1 | sed 's/#//' | perl -pe 's/([^\t]+):([0-9]+)\t*/\\1\t\\2\n/g'")), header=F, col.names=c("Name", "dim"))
...
profilePlot <- ggplot(profiles, aes(x=variable, y=mean, group=Category)) + geom_line(aes(colour=Category)) + xscale + theme_bw()
